...
#    under the License.
"""Brocade EDGE Driver implements TELNET for
...
ROUTER_EDGE_CONFIGURATION = "router mpls\r"
...
class BrocadeMLXDriver():
    ...
    def connect(self, switch):
        ...
        LOG.debug("Connect success to host %s", {host: TELNET_PORT})
        return self.mgr
    ...
    def validate_terminal(self, mgr):
        ...
        if pattern.match(response):
            required_auth = 'enAuth'
        pattern = re.compile(SUPER_USER_AUTH, re.IGNORECASE)
        if pattern.match(response):
            required_auth = 'superAuth'
        LOG.debug("Device in %(required_auth)s authentication mode", {
    ...
    def _enter_edge_configuration(self, mgr):
        """Enter configuration mode.

        First it enters enable mode and configuration mode and
        then to router mpls mode.
        """
        ...
        self._read_response(mgr)
        mgr.write(ROUTER_EDGE_CONFIGURATION)
        self._read_response(mgr)
    ...
    def configure_vpls(self, switch, vpls_id, vplsname, deviceType, vlans):
        ...
        self._enter_edge_configuration(mgr)
    ...
    def find_hardware(self, switch):
        ...
        self.hardware_type = "MLX"
        mgr = self.connect(switch)
        mgr.write("sh chassis " + "\n\r")
        mgr.write(" " + "\n\r")
        message = mgr.read_until("#", 8)
        response = re.split("\r\n\r\n|\r\n", message)
        chassis_type = re.compile(MLX_XMR_PATTERN, re.IGNORECASE)
        cesr_type = re.compile(CER_PATTERN, re.IGNORECASE)
        if chassis_type.match(response[1]):
            self.hardware_type = "MLX"
        elif cesr_type.match(response[1]):
            self.hardware_type = "CER"
        self.close_session()
