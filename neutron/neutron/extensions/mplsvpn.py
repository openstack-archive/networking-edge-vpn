...
class DuplicateMPLSVPNForTenant(qexception.InvalidInput):
    message = _("MPLSVPN service %(mplsvpn_id)s already exists for tenant %(tenant_id)s")


class AttachmentCircuitNotFound(qexception.NotFound):
    message = _("AttachmentCircuit %(attachmentcircuit_id)s could not be found")


class DuplicateAttachmentCircuitForTenant(qexception.InvalidInput):
    message = _("Attachment circuit %(attachmentcircuit_id)s already exists for tenant %(tenant_id)s")
