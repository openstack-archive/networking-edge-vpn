...
class AttachmentCircuit(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    """Represents a Attached Circuit."""
    name = sa.Column(sa.String(255))
    network_type = sa.Column(sa.Enum("L2", "L3",
                             name="network_type"),
                             nullable=False)
    provider_edge_id = sa.Column(sa.String(255), sa.ForeignKey('provideredges.id'), nullable=False)
    networks = orm.relationship("ACNetworkAssociation", backref="attachmentcircuits",
...
class ACNetworkAssociation(model_base.BASEV2,
    ...
    attachmentcircuit_id = sa.Column(sa.String(36), sa.ForeignKey("attachmentcircuits.id"), primary_key=True)
    network_id = sa.Column(sa.String(36), sa.ForeignKey("networks.id"), primary_key=True)


class ACMPLSVPNAssociation(model_base.BASEV2,
                           models_v2.HasId,
                           models_v2.HasStatusDescription):
    mplsvpn_id = sa.Column(sa.String(36), sa.ForeignKey("mplsvpns.id"), primary_key=True)
    attachmentcircuit_id = sa.Column(sa.String(36), sa.ForeignKey("attachmentcircuits.id"), primary_key=True)
...
class MPLSVPN(model_base.BASEV2, models_v2.HasId, models_v2.HasTenant):
    ...
    tunnel_backup = sa.Column(sa.Enum("frr", "Secondary",
                              ...
                              nullable=False)
    qos = sa.Column(sa.Enum("Gold", "Silver", "Bronze", 
                            name="qos"),
                    nullable=False)
    bandwidth = sa.Column(sa.Integer, nullable=False)
    vpn_id = sa.Column(sa.String(36), default="", nullable=False)
    attachment_circuits = orm.relationship("ACMPLSVPNAssociation", backref="mplsvpns",
                                           cascade="all", lazy="joined")
...
class MPLSVPNPluginDb(MPLSVPNPluginBase, base_db.CommonDbMixin):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                if issubclass(model, MPLSVPN):
                    ...
                    raise mplsvpnextension.AttachmentCircuitNotFound(attachmentcircuit_id=v_id)
                elif issubclass(model, ProviderEdge):
                    raise mplsvpnextension.ProviderEdgeNotFound(provideredge_id=v_id)
        return r
    ...
    def _modify_mplsvpn_ac_associations(self, context, mplsvpn_id, ac_id_list):
        with context.session.begin(subtransactions=True):
            ...
            """For each association delete if not in updated attachment circuit list for mplsvpn."""
            for assoc in assocs:
                if not assoc.attachmentcircuit_id in ac_id_list:
                    assoc_db = self._get_resource(context, ACMPLSVPNAssociation, assoc.id)
            ...
            for attachmentcircuit_id in ac_id_list:
                ...
                if not assoc:
                    assoc = ACMPLSVPNAssociation(mplsvpn_id=mplsvpn_id,
    ...
    def _make_mplsvpn_dict(self, mplsvpn, fields=None):
        ...
        res['attachment_circuits'] = [n['attachmentcircuit_id'] for n in mplsvpn.attachment_circuits]
    ...
    def create_mplsvpn(self, context, mplsvpn):
        ...
        mplsvpn_db = context.session.query(MPLSVPN).filter_by(tenant_id=tenant_id).first()
        if mplsvpn_db:
            mplsvpn_id = mplsvpn_db['id']
            raise mplsvpnextension.DuplicateMPLSVPNForTenant(mplsvpn_id=mplsvpn_id, tenant_id=tenant_id)
        ...
        with context.session.begin(subtransactions=True):
            ...
            self._modify_mplsvpn_ac_associations(context, mplsvpns_db.id, mplsvpns['attachment_circuits'])
    ...
    def update_mplsvpn(self, context, mplsvpn_id, mplsvpn):
        ...
        with context.session.begin(subtransactions=True):
            if 'attachment_circuits' in mplsvpns:
                ...
                self._modify_mplsvpn_ac_associations(context, mplsvpn_id, ac_id_list)
            mplsvpn_db = self._get_resource(context, MPLSVPN, mplsvpn_id)
    ...
    def get_network_ports(self, context, network_id):
        ...
        return session.query(models_v2.Port).filter_by(network_id=network_id,
    ...
    def _modify_ac_networks_associations(self, context, attachmentcircuit_id, network_id_list):
        with context.session.begin(subtransactions=True):
            assoc_qry = context.session.query(ACNetworkAssociation)
            assocs = assoc_qry.filter_by(attachmentcircuit_id=attachmentcircuit_id).all()
            for assoc in assocs:
                if not assoc.network_id in network_id_list:
                    assoc_db = self._get_resource(context, ACNetworkAssociation, assoc.id)
                    context.session.delete(assoc_db)
            attachmentcircuit_db = self._get_resource(context, AttachmentCircuit, attachmentcircuit_id)
            assoc_qry = context.session.query(ACNetworkAssociation)
            for network_id in network_id_list:
                assoc = assoc_qry.filter_by(attachmentcircuit_id=attachmentcircuit_id,
                                            network_id=network_id).first()
                if not assoc:
                    assoc = ACNetworkAssociation(attachmentcircuit_id=attachmentcircuit_id,
                                                 network_id=network_id,
                                                 status=constants.ACTIVE)
                    attachmentcircuit_db.networks.append(assoc)

    def add_network_to_attachmentcircuit(context, attachmentcircuit_id, network_id):
        with context.session.begin(subtransactions=True):
            session_qry = context.session.query(AttachmentCircuit)
            attachmentcircuit_db = session_qry.filter_by(id=attachmentcircuit_id).first()
            assoc_qry = context.session.query(ACNetworkAssociation)
            assoc = assoc_qry.filter_by(attachmentcircuit_id=attachmentcircuit_id,
                                        network_id=network_id).first()
            if not assoc:
    ...
    def remove_network_from_attachmentcircuit(context, attachmentcircuit_id, network_id):
        with context.session.begin(subtransactions=True):
            assoc_qry = context.session.query(ACNetworkAssociation)
            assoc = assoc_qry.filter_by(attachmentcircuit_id=attachmentcircuit_id,
                                        network_id=network_id).first()
    ...
    def create_attachment_circuit(self, context, attachment_circuit):
        ...
        attachmentcircuit_db = context.session.query(AttachmentCircuit).filter_by(tenant_id=tenant_id).first()
        if attachmentcircuit_db:
            ac_id = attachmentcircuit_db['id']
            raise mplsvpnextension.DuplicateAttachmentCircuitForTenant(attachmentcircuit_id=ac_id,
                                                                       tenant_id=tenant_id)
        with context.session.begin(subtransactions=True):
            attachmentcircuits_db = AttachmentCircuit(id=uuidutils.generate_uuid(),
            ...
            self._modify_ac_networks_associations(context, attachmentcircuits_db.id, attachmentcircuits['networks'])
        return self._make_attachmentcircuit_dict(attachmentcircuits_db)

    def update_attachment_circuit(self, context, attachmentcircuit_id, attachmentcircuit):
        attachmentcircuits = attachmentcircuit['attachment_circuit']
        with context.session.begin(subtransactions=True):
            if 'networks' in attachmentcircuits:
                self._modify_ac_networks_associations(context, attachmentcircuit_id, attachmentcircuits['networks'])
            attachmentcircuits_db = self._get_resource(context, AttachmentCircuit, attachmentcircuit_id)
        return self._make_attachmentcircuit_dict(attachmentcircuits_db)
    ...
    def _get_attachment_circuit(self, context, attachmentcircuit_id):
        return self._get_resource(context, AttachmentCircuit, attachmentcircuit_id)

    def get_attachment_circuit(self, context, attachmentcircuit_id, fields=None):
        attachmentcircuits_db = self._get_resource(context, AttachmentCircuit, attachmentcircuit_id)
    ...
    def get_attachmentcircuit_for_tenant(context, tenant_id):
        session = context.session
        return session.query(AttachmentCircuit).filter_by(tenant_id=tenant_id).first()

    def get_attachmentcircuit(context, attachmentcircuit_id):
        session = context.session
        return session.query(AttachmentCircuit).filter_by(id=attachmentcircuit_id).first()

    def get_vlans_for_attachment_circuit_id(context, attachmentcircuit_id):
        vlans = []
        session = context.session
        attachment_circuit = session.query(AttachmentCircuit).filter_by(id=attachmentcircuit_id).first()
        network_id_list = [n['network_id'] for n in attachment_circuit.networks]
        for network_id in network_id_list:
    ...
    def get_mplsvpn_for_attachment_circuit(context, attachmentcircuit_id):
        mplsvpn = None
        assoc_qry = context.session.query(ACMPLSVPNAssociation)
        assoc = assoc_qry.filter_by(attachmentcircuit_id=attachmentcircuit_id).first()
        if assoc:
            mplsvpn = context.session.query(MPLSVPN).filter_by(id=assoc.mplsvpn_id).first()
        return mplsvpn
    ...
    def update_provider_edge(self, context, provideredge):
        with context.session.begin(subtransactions=True):
            provideredges_db = self._get_resource(context, ProviderEdge, provideredge['id'])
            provideredges_db.update(provideredge)
        return self._make_provideredge_dict(provideredges_db)

    def delete_provider_edge(self, context, provideredge_id):
        with context.session.begin(subtransactions=True):
            provideredges_db = self._get_resource(context, ProviderEdge, provideredge_id)
