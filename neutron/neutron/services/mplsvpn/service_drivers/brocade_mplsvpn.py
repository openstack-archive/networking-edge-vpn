...
from neutron.services.mplsvpn.device_drivers import brocade_mlxdriver as mlxvpndriver
...
MPLSVPN = 'mplsvpn'
MLX_DRIVER = 'neutron.services.mplsvpn.device_drivers.brocade_mlxdriver.BrocadeMLXDriver'
...
class BrocadeMPLSVPNDriver(service_drivers.MplsVpnDriver):
    ...
    def create_mplsvpn(self, context, mplsvpn):
        ...
        service_type = MPLSVPN 
        if not vpls_id:
            msg = (_("No vpn_id specified for '%s' service, exiting") %
                service_type)
            LOG.error(msg)
        ...
        with context.session.begin(subtransactions=True):
            ...
            for ac_id in ac_id_list:
                vlans.extend(db.get_vlans_for_attachment_circuit_id(context, ac_id))
            try:
                ...
                """If MCT cluster peer switch has been defined in config file, configure VPLS."""
                if self._cluster_peer_switch:
                    deviceType = self._driver.find_hardware(self._cluster_peer_switch)
                    self._driver.configure_vpls(self._cluster_peer_switch, vpls_id,
                                                vpls_name, deviceType, vlans)
            except Exception:
                raise Exception(_("Brocade plugin raised exception, check logs"))
    ...
    def update_mplsvpn(self, context, old_mplsvpn, new_mplsvpn):
        ...
        with context.session.begin(subtransactions=True):
            ...
            for ac_id in old_ac_id_list:
                old_vlans.extend(db.get_vlans_for_attachment_circuit_id(context, ac_id))
            new_ac_id_list = new_mplsvpn['attachment_circuits']
            for ac_id in new_ac_id_list:
                new_vlans.extend(db.get_vlans_for_attachment_circuit_id(context, ac_id))
            vlans_to_add = []
            vlans_to_remove = []
            ...
            if vlans_to_add or vlans_to_remove:
                LOG.debug("vlans_to_add: %s, vlans_to_remove: %s", vlans_to_add, vlans_to_remove)
                try:
                    self._driver.modify_vlans_for_vpls(self._switch, new_mplsvpn['vpn_id'],
                                                       new_mplsvpn['name'], vlans_to_add, vlans_to_remove)
                    if self._cluster_peer_switch:
                        self._driver.modify_vlans_for_vpls(self._cluster_peer_switch, new_mplsvpn['vpn_id'],
                                                           new_mplsvpn['name'], vlans_to_add, vlans_to_remove)
                except Exception:
                    raise Exception(_("Brocade plugin raised exception, check logs"))
        return new_mplsvpn
    ...
    def delete_mplsvpn(self, context, mplsvpn):
        with context.session.begin(subtransactions=True):
            ...
            try:
                ...
                if self._cluster_peer_switch:
                    self._driver.delete_vpls(self._cluster_peer_switch, vplsname, vpn_id)
            except Exception:
                raise Exception(_("Brocade MLX plugin exception, check logs"))

    def update_attachment_circuit(self, context, old_attachment_circuit, new_attachment_circuit):
        old_vlans = []
        new_vlans = []
        with context.session.begin(subtransactions=True):
            mplsvpn = db.get_mplsvpn_for_attachment_circuit(context, old_attachment_circuit['id'])
            if mplsvpn:
                old_vlans = db.get_vlans_for_attachment_circuit(context, old_attachment_circuit)
                new_vlans = db.get_vlans_for_attachment_circuit(context, new_attachment_circuit)
                ...
                if vlans_to_add or vlans_to_remove:
                    LOG.debug("vlans_to_add: %s, vlans_to_remove: %s", vlans_to_add, vlans_to_remove)
                    try:
                        self._driver.modify_vlans_for_vpls(self._switch, mplsvpn['vpn_id'],
                                                           mplsvpn['name'], vlans_to_add, vlans_to_remove)
                        if self._cluster_peer_switch:
                            self._driver.modify_vlans_for_vpls(self._cluster_peer_switch, mplsvpn['vpn_id'],
                                                               mplsvpn['name'], vlans_to_add, vlans_to_remove)
                    except Exception:
                        raise Exception(_("Brocade plugin raised exception, check logs"))
        return new_attachment_circuit
