...
class MPLSVPNDriverPlugin(MPLSVPNPlugin):
    ...
    def __init__(self):
        ...
        LOG.info(_("MPLSVPN plugin using service driver: %s"), default_provider)
    ...
    def create_mplsvpn(self, context, mplsvpn):
        ...
        mplsvpn = super(MPLSVPNDriverPlugin, self).create_mplsvpn(context, mplsvpn)
        """ Get service driver. """
        driver = self._get_driver()
        """ Call service driver to create mplsvpn instance on device. """
        mplsvpn = driver.create_mplsvpn(context, mplsvpn)
        super(MPLSVPNDriverPlugin, self).update_mplsvpn_status_and_name(context, mplsvpn)
        return mplsvpn
    ...
    def create_attachment_circuit(self, context, attachment_circuit):
        """ Call super to create row in DB. """
        attachcircuit = super(MPLSVPNDriverPlugin, self).create_attachment_circuit(context, attachment_circuit)
        return attachcircuit
 
    def update_attachment_circuit(self, context, attachmentcircuit_id, attachment_circuit):
        ...
        new_ac = super(MPLSVPNDriverPlugin,
                       self).update_attachment_circuit(context,
                                                       ...
                                                       attachment_circuit)
        driver = self._get_driver()
        attachment_circuit = driver.update_attachment_circuit(context, old_ac, new_ac)
        return new_ac

    def delete_attachment_circuit(self, context, attachmentcircuit_id):
        """ Get AttachmentCircuit instances from DB by the given id. """
        attachmentcircuit = self._get_attachment_circuit(context, attachmentcircuit_id)
        """ Delete the instance from the DB. """
        super(MPLSVPNDriverPlugin, self).delete_attachment_circuit(context, attachmentcircuit_id)

    def create_provider_edge(self, context, provider_edge):
        """ Call super to create row in DB. """
        provideredge = super(MPLSVPNDriverPlugin, self).create_provider_edge(context, provider_edge)
        return provideredge
        
    def delete_provider_edge(self, context, provideredge_id):
        """ Get Provider Edge instances from DB by the given id. """
        provideredge = self._get_provider_edge(context, provideredge_id)
        """ Delete the instance from the DB. """
        super(MPLSVPNDriverPlugin, self).delete_provider_edge(context, provideredge_id)
