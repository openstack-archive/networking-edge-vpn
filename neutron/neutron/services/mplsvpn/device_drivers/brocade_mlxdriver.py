...
class BrocadeMLXDriver():
    ...
    Handles life-cycle management of Neutron network (leverages VPLS on MLX)
    """
    ...
    def configure_vpls(self, switch, vpls_id, vplsname, deviceType, vlans):
        ...
        for vlan_id in vlans:
            ...
            cliCmdOutput = self._read_response(mgr)
            LOG.debug("output %s", cliCmdOutput)
            for port in ports:
    ...
    def modify_vlans_for_vpls(self, switch, vpls_id, vplsname, vlans_to_add, vlans_to_remove):
    ...
    def find_hardware(self, switch):
        ...
        if chassisType.match(respone[1]):
            self.hardwareType = "MLX"
        elif cesrType.match(respone[1]):
